...
- **Database:** Encrypted SQLite using AES-256-GCM.
- **Key Management:** Private keys for Blockchain/Web3 and Provider APIs are stored in the OS-native keychain (Keyring/Keychain/Secret Service) where possible, or encrypted via Argon2id key derivation.
- **No Cleartext:** Your API keys are never stored in cleartext on the filesystem.
...
- [x] **Encryption at Rest:** All local SQLite databases are encrypted using AES-256-GCM.
- [x] **Secure Key Storage:** Provider API keys are stored in the OS-native keychain (Keyring/Keychain), never in cleartext.
