...
struct ZaiMessage {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    tool_calls: Option<Vec<ZaiToolCallMsg>>,
}

#[derive(Debug, Serialize)]
struct ZaiToolCallMsg {
    id: String,
    #[serde(rename = "type")]
    call_type: String,
...
/// Z.AI provider (GLM-5.2, GLM-4.6, GLM-4.5-Air, etc.).
...
impl ZaiProvider {
    ...
    /// Convert generic messages to the Z.AI wire format.
    fn convert_messages(
        ...
        for m in messages {
            ...
            {
                let tc_msgs: Vec<ZaiToolCallMsg> = calls
                    .iter()
                    .map(|c| ZaiToolCallMsg {
                        id: c.id.clone(),
    ...
    fn build_body(&self, request: &ChatRequest, stream: bool) -> ZaiChatRequest {
        ZaiChatRequest {
...
impl AiProvider for ZaiProvider {
    ...
    async fn get_models(&self) -> Vec<ModelInfo> {
        let mut static_models: Vec<ModelInfo> =
            crate::model_registry::models_for_provider(ProviderType::Zai)
                .into_iter()
                .cloned()
                .collect();
        ...
        if let Ok(key) = self.require_key() {
            if let Ok(live) = super::zai_catalog::fetch_zai_models(key).await {
    ...
    async fn chat(&self, request: &ChatRequest) -> Result<ChatResponse, ProviderError> {
        ...
        let choice = data
            .choices
            .first()
            .ok_or_else(|| ProviderError::Other("No choices in Z.AI response".into()))?;
...
mod tests {
    ...
    fn build_body_standard() {
        let provider = ZaiProvider::new("zai-test".into());
        let req = sample_request("glm-4.6");
    ...
    fn build_body_with_system_prompt() {
        let provider = ZaiProvider::new("zai-test".into());
        let mut req = sample_request("glm-4.6");
    ...
    fn provider_metadata() {
        let provider = ZaiProvider::new("zai-test".into());
        assert_eq!(provider.provider_type(), ProviderType::Zai);
    ...
    async fn is_available_without_key() {
        let provider = ZaiProvider::new(String::new());
        assert!(!provider.is_available().await);
    }
    ...
    fn request_body_serializes_correctly() {
        let provider = ZaiProvider::new("zai-test".into());
        let req = sample_request("glm-5.2");
        let body = provider.build_body(&req, false);
