//! xAI / Grok provider.
//!
//! xAI exposes an OpenAI-compatible API at `api.x.ai/v1`. Streaming uses
...
/// xAI Grok API provider (Grok-3, Grok-3 Mini, Grok-2, etc.).
...
impl XaiProvider {
    ...
    /// Convert generic messages to the xAI wire format.
    fn convert_messages(messages: &[ChatMessage], system_prompt: Option<&str>) -> Vec<XaiMessage> {
    ...
    fn build_body(&self, request: &ChatRequest, stream: bool) -> XaiChatRequest {
        XaiChatRequest {
...
impl AiProvider for XaiProvider {
    ...
    async fn get_models(&self) -> Vec<ModelInfo> {
        crate::model_registry::models_for_provider(ProviderType::XAI)
            .into_iter()
            .cloned()
            .collect()
    }
    ...
    async fn chat(&self, request: &ChatRequest) -> Result<ChatResponse, ProviderError> {
        ...
        let choice = data
            .choices
            .first()
            .ok_or_else(|| ProviderError::Other("No choices in xAI response".into()))?;
...
mod tests {
    ...
    fn build_body_standard() {
        let provider = XaiProvider::new("xai-test".into());
        let req = sample_request("grok-3");
    ...
    fn build_body_with_system_prompt() {
        let provider = XaiProvider::new("xai-test".into());
        let mut req = sample_request("grok-3");
    ...
    async fn is_available_without_key() {
        let provider = XaiProvider::new(String::new());
        assert!(!provider.is_available().await);
    }
    ...
    fn request_body_serializes_correctly() {
        let provider = XaiProvider::new("xai-test".into());
        let req = sample_request("grok-3-mini");
        let body = provider.build_body(&req, false);
