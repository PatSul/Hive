    ...
    pub fn sample() -> Self {
        ...
        Self {
            ...
            run_history: vec![
                RunHistoryEntry::new(
                    "run-007",
                    "Implement Agent Monitor panel with full UI",
                    4,
                ...
                RunHistoryEntry::new(
                    "run-006",
                    "Add cost tracking dashboard with model breakdown",
                    3,
                ...
                RunHistoryEntry::new(
                    "run-005",
                    "Fix token launch wizard wallet creation flow",
                    5,
                    AgentSystemStatus::Idle,
                    1.34,
                    "11:42",
                ...
                RunHistoryEntry::new(
                    "run-004",
                    "Security audit of IPC handlers and preload bridge",
                    6,
                    AgentSystemStatus::Idle,
                    2.10,
                    "09:30",
                ...
                RunHistoryEntry::new(
                    "run-003",
                    "Refactor messaging hub for Telegram provider",
                    3,
...
impl MonitorPanel {
    ...
    fn provider_status_section(data: &MonitorData, theme: &HiveTheme) -> impl IntoElement {
        ...
        let mut container = Self::section("AI Provider Status", theme).child(
            div()
    ...
    fn provider_card(provider: &ProviderStatus, theme: &HiveTheme) -> impl IntoElement {
        ...
        div()
            ...
            .child(Self::provider_status_row(
                status_text,
                &latency_text,
                dot_color,
                theme,
    ...
    fn runtime_stats_section(data: &MonitorData, theme: &HiveTheme) -> impl IntoElement {
        ...
        div()
            ...
            .child(Self::simple_card(
                ...
                if data.request_queue_length > 10 {
                    theme.accent_yellow
                } else {
                    theme.accent_cyan
                },
    ...
    fn active_agents_section(data: &MonitorData, theme: &HiveTheme) -> impl IntoElement {
        ...
        if data.active_agents.is_empty() {
            container = container.child(Self::empty_state(
                "No agents running. Start a HiveMind task to see activity here.",
                theme,
            ));
        } else {
    ...
    fn run_history_section(data: &MonitorData, theme: &HiveTheme) -> impl IntoElement {
        let mut container = Self::section("Run History", theme).child(Self::history_header(theme));

        if data.run_history.is_empty() {
            container = container.child(Self::empty_state(
                "No orchestration runs yet -- history will appear here.",
                theme,
            ));
        } else {
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .text_size(theme.font_size_sm)
                    .text_color(theme.text_secondary)
                    .child(entry.agents_used.to_string()),
            )
