...
impl RoutingPanel {
    ...
    fn tier_premium_section(theme: &HiveTheme) -> Div {
        div()
            .flex()
            .flex_col()
            .child(Self::tier_section_header(
                "Premium",
                theme.accent_red,
                theme,
            ))
    ...
    fn perf_row(entry: &ProviderStatusEntry, theme: &HiveTheme) -> impl IntoElement {
        ...
        div()
            ...
            .child(Self::cell_fixed(
                &format!("{}", entry.requests),
                px(60.0),
                theme.text_secondary,
                theme,
            ))
            .child(Self::cell_fixed(
                &format!("{}", entry.failures),
                px(72.0),
                failures_color,
                theme,
            ))
    ...
    fn chain_row(tier: &str, color: Hsla, chain: &[&str], theme: &HiveTheme) -> impl IntoElement {
        let mut row = div()
            .flex()
            .flex_row()
            .items_center()
