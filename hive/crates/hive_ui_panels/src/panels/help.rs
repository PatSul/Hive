...
fn render_features_overview(theme: &HiveTheme) -> AnyElement {
    ...
    let mut grid = div().flex().flex_col().gap(theme.space_2);
    ...
    while i < features.len() {
        ...
        row = row.child(div().flex_1().child(feature_card(
            features[i].0,
            features[i].1,
            features[i].2,
            theme,
    ...
    for (category, libs) in tier2_data {
        ...
        while i < libs.len() {
            ...
            row = row.child(
                div()
                    .flex_1()
                    .child(credit_card(libs[i].0, libs[i].1, theme)),
            );
            if i + 1 < libs.len() {
                row = row.child(div().flex_1().child(credit_card(
                    libs[i + 1].0,
                    libs[i + 1].1,
                    theme,
                )));
