...
fn render_wallet_step(data: &TokenLaunchData, theme: &HiveTheme) -> AnyElement {
    ...
    let wallet_display: AnyElement = match &data.wallet_address {
        ...
        None => div()
            ...
            .py(theme.space_6)
            .gap(theme.space_2)
            .child(div().text_size(px(32.0)).child("\u{1F4B3}"))
            .child(
                div()
                    .text_size(theme.font_size_base)
                    .text_color(theme.text_muted)
    ...
    let fund_status: AnyElement = if data.wallet_address.is_some() {
        ...
        let (label, color) = if sufficient {
            ...
            (
                "Insufficient funds -- please add funds before deploying",
                theme.accent_red,
            )
        };
        div()
...
fn render_deploy_step(data: &TokenLaunchData, theme: &HiveTheme) -> AnyElement {
    ...
    let name_display = if data.token_name.is_empty() {
        "--"
    } else {
        &data.token_name
    };
    let symbol_display = if data.token_symbol.is_empty() {
        "--"
    } else {
        &data.token_symbol
    };
    let supply_display = if data.total_supply.is_empty() {
        "--"
    } else {
        &data.total_supply
    };
    ...
    let status_display: AnyElement = match &data.deploy_status {
        ...
        DeployStatus::Deploying => div()
            ...
            .into_any_element(),
        DeployStatus::Success(contract_addr) => div()
            .flex()
            .flex_col()
            .items_center()
            .gap(theme.space_2)
...
fn render_navigation(data: &TokenLaunchData, theme: &HiveTheme) -> AnyElement {
    ...
    let back_button: AnyElement = if show_back {
        ...
        div()
            ...
            .cursor_pointer()
            .on_mouse_down(MouseButton::Left, move |_event, window, cx| {
                window.dispatch_action(Box::new(TokenLaunchSetStep { step: back_step }), cx);
            })
            .child("\u{2190} Back")
            .into_any_element()
    } else {
    ...
    let next_button: AnyElement = if is_deploy_step {
        ...
        div()
            ...
            .cursor_pointer()
            .on_mouse_down(MouseButton::Left, move |_event, window, cx| {
                window.dispatch_action(Box::new(TokenLaunchSetStep { step: next_step }), cx);
            })
            .child("Next \u{2192}")
            .into_any_element()
    } else {
