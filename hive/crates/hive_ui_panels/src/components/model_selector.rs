...
impl Render for ModelSelectorView {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let display = info
            .map(|m| m.name.as_str())
            .unwrap_or(if self.current_model.is_empty() {
                "Select a model"
            } else {
        ...
        div()
            ...
            .when(is_open, |el| el.child(self.render_dropdown(theme, cx)))
    }
}

impl ModelSelectorView {
    fn render_dropdown(&self, theme: &HiveTheme, cx: &mut Context<Self>) -> impl IntoElement {
        let provider_order = [
        ...
        div()
            ...
            // Search input at top
            .child(
                div().px(theme.space_2).py(theme.space_1).child(
