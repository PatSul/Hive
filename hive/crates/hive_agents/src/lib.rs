...
pub use activity::{
    ActivityEntry, ActivityEvent, ActivityFilter, ActivityLog, ActivityService, ApprovalDecision,
    ApprovalGate, ApprovalRequest, ApprovalRule, BudgetConfig, BudgetDecision, BudgetEnforcer,
    CostSummary, ExhaustAction, FileOp, Notification, NotificationKind, NotificationService,
    OperationType, PauseReason, RuleTrigger,
};
pub use auto_commit::{AutoCommitConfig, AutoCommitService, CommitResult};
pub use automation::{
    ActionType, AutomationService, BUILTIN_DOGFOOD_WORKFLOW_ID, Condition, ConditionOp,
    TriggerType, USER_WORKFLOW_DIR, Workflow, WorkflowLoadReport, WorkflowRunResult,
...
pub use personas::{
    Persona, PersonaKind, PersonaRegistry, PromptOverride, execute_with_persona,
    execute_with_persona_model,
};
pub use pipeline::{PipelineConfig, PipelineStage, TaskPipeline, ValidationGateKind};
pub use plugin_manager::PluginManager;
pub use plugin_types::{
    CachedVersion, InstalledCommand, InstalledPlugin, ParsedCommand, ParsedSkill, PluginAuthor,
    PluginCache, PluginManifest, PluginPreview, PluginSkill, PluginSource, PluginStore,
    UpdateAvailable,
};
