...
#[derive(Serialize)]
pub struct LoginRequest { pub email: String }
...
#[derive(Debug, Deserialize)]
pub struct UsageInfo { pub input_tokens: i32, pub output_tokens: i32 }
...
#[derive(Debug, Deserialize)]
pub struct SseChunk { pub chunk: String }
...
impl CloudClient {
    pub fn new(api_url: Option<&str>, jwt: Option<&str>) -> Self {
        ...
        Self { base_url, jwt: jwt.map(|s| s.to_string()), client: reqwest::Client::new() }
    }
    ...
    pub async fn login(&self, email: &str) -> Result<TokenPair> {
        let resp = self.client.post(format!("{}/auth/login", self.base_url))
            .json(&LoginRequest { email: email.to_string() })
            .send().await.context("Failed to connect to Hive Cloud")?;
        if !resp.status().is_success() {
            let s = resp.status(); let b = resp.text().await.unwrap_or_default();
            bail!("Login failed ({s}): {b}");
        }
        resp.json::<TokenPair>().await.context("Failed to parse login response")
    }

    pub async fn get_account(&self) -> Result<AccountInfo> {
        let resp = self.auth_get("/account")?.send().await
            .context("Failed to connect to Hive Cloud")?;
        if !resp.status().is_success() {
            let s = resp.status(); let b = resp.text().await.unwrap_or_default();
            bail!("Failed to get account ({s}): {b}");
        }
        resp.json::<AccountInfo>().await.context("Failed to parse account response")
    }

    pub async fn get_usage(&self) -> Result<UsageSummary> {
        let resp = self.auth_get("/account/usage")?.send().await
            .context("Failed to connect to Hive Cloud")?;
        if !resp.status().is_success() {
            let s = resp.status(); let b = resp.text().await.unwrap_or_default();
            bail!("Failed to get usage ({s}): {b}");
        }
        resp.json::<UsageSummary>().await.context("Failed to parse usage response")
    }

    pub async fn chat_completion(
        &self, model: &str, messages: &[ChatMessage],
    ) -> Result<reqwest::Response> {
        let req = ChatRequest {
            model: model.to_string(), messages: messages.to_vec(),
            max_tokens: Some(4096), temperature: Some(0.7), stream: true,
        };
        let resp = self.auth_post("/gateway/v1/chat")?
            .json(&req).send().await.context("Failed to connect to AI gateway")?;
        if !resp.status().is_success() {
            let s = resp.status(); let b = resp.text().await.unwrap_or_default();
    ...
    pub async fn list_models(&self) -> Result<Vec<ModelInfo>> {
        let resp = self.client.get(format!("{}/gateway/v1/models", self.base_url))
            .send().await.context("Failed to connect to Hive Cloud")?;
        if !resp.status().is_success() {
            let s = resp.status(); let b = resp.text().await.unwrap_or_default();
            bail!("Failed to list models ({s}): {b}");
        }
        resp.json::<Vec<ModelInfo>>().await.context("Failed to parse models response")
    }

    pub async fn sync_push(&self, key: &str, data: &[u8]) -> Result<()> {
        let resp = self.auth_put(&format!("/v1/sync/blobs/{}", key))?
            .header("Content-Type", "application/octet-stream")
            .body(data.to_vec()).send().await.context("Failed to push blob")?;
        if !resp.status().is_success() {
            let s = resp.status(); let b = resp.text().await.unwrap_or_default();
    ...
    pub async fn sync_pull(&self, key: &str) -> Result<Vec<u8>> {
        let resp = self.auth_get(&format!("/v1/sync/blobs/{}", key))?
            .send().await.context("Failed to pull blob")?;
        if !resp.status().is_success() {
            let s = resp.status(); let b = resp.text().await.unwrap_or_default();
    ...
    pub async fn sync_manifest(&self) -> Result<ManifestResponse> {
        let resp = self.auth_get("/v1/sync/manifest")?.send().await
            .context("Failed to get manifest")?;
        if !resp.status().is_success() {
            let s = resp.status(); let b = resp.text().await.unwrap_or_default();
            bail!("Sync manifest failed ({s}): {b}");
        }
        resp.json::<ManifestResponse>().await.context("Failed to parse manifest")
    }
}
