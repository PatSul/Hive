...
fn test_from_index_valid() {
    ...
    assert_eq!(Panel::from_index(5), Some(Panel::Kanban));
    assert_eq!(Panel::from_index(6), Some(Panel::Monitor));
    assert_eq!(Panel::from_index(7), Some(Panel::Logs));
    assert_eq!(Panel::from_index(8), Some(Panel::Costs));
