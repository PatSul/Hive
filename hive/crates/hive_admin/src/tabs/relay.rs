...
pub fn draw(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    let summary = Paragraph::new(format!("Active Rooms: {}  |  Connected Devices: {}", stats.active_rooms, stats.connected_devices))
        .block(Block::default().borders(Borders::ALL).title(" Relay Status "))
        .style(Style::default().fg(CYAN));
    frame.render_widget(summary, chunks[0]);

    let header = Row::new(vec!["Room ID", "Participants", "Created", "Bytes Transferred"])
        .style(Style::default().fg(CYAN).add_modifier(Modifier::BOLD));
    let rows: Vec<Row> = stats.rooms.iter().map(|r| {
        Row::new(vec![r.room_id.clone(), r.participants.to_string(), r.created_at.format("%H:%M:%S").to_string(), format_bytes(r.bytes_transferred)])
    }).collect();
    let widths = [Constraint::Min(15), Constraint::Length(14), Constraint::Length(12), Constraint::Length(18)];
