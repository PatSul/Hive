...
pub fn draw(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    let header = Row::new(vec!["ID", "Email", "Tier", "Created", "Last Login", "Tokens Used"])
        .style(Style::default().fg(CYAN).add_modifier(Modifier::BOLD));

    let rows: Vec<Row> = filtered.iter().map(|u| {
    ...
    let widths = [Constraint::Length(10), Constraint::Min(20), Constraint::Length(6), Constraint::Length(12), Constraint::Length(18), Constraint::Length(12)];
