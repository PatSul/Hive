...
pub fn draw(frame: &mut Frame, app: &App, area: Rect) {
    ...
    frame.render_widget(
        stat_card("Total Users", &stats.total_users.to_string()),
        row1[0],
    );
    frame.render_widget(
        stat_card("Free Tier", &stats.free_users.to_string()),
        row1[1],
    );
    frame.render_widget(stat_card("Pro Tier", &stats.pro_users.to_string()), row1[2]);
    frame.render_widget(
        stat_card("Team Tier", &stats.team_users.to_string()),
        row1[3],
    );
    ...
    frame.render_widget(
        stat_card("Est. Revenue", &format!("${:.0}", stats.revenue_estimate)),
        row2[0],
    );
    frame.render_widget(
        stat_card(
            "Requests Today",
            &format!("{}", stats.gateway_requests_today),
        ),
        row2[1],
    );
    frame.render_widget(
        stat_card(
            "Relay Connections",
            &stats.active_relay_connections.to_string(),
        ),
        row2[2],
    );
    frame.render_widget(
        stat_card("Sync Storage", &format_bytes(stats.sync_storage_bytes)),
        row2[3],
    );
    ...
    let summary = Paragraph::new(vec![Line::from(vec![
        ...
        Span::styled(
            format!("{}", stats.gateway_requests_month),
