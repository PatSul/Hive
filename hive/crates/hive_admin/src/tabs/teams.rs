...
pub fn draw(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    let header = Row::new(vec!["ID", "Name", "Members", "Plan", "Monthly Cost", "Usage"])
        .style(Style::default().fg(CYAN).add_modifier(Modifier::BOLD));
    let rows: Vec<Row> = app.teams.iter().map(|t| {
        Row::new(vec![
            t.id.clone(),
            t.name.clone(),
    ...
    let widths = [Constraint::Length(10), Constraint::Min(20), Constraint::Length(10), Constraint::Length(8), Constraint::Length(14), Constraint::Length(10)];
    ...
    let detail = if let Some(idx) = selected_idx {
        if let Some(team) = app.teams.get(idx) {
            let mut lines = vec![
                Line::from(vec![
                    Span::styled(format!("{}", team.name), Style::default().fg(CYAN).add_modifier(Modifier::BOLD)),
            ...
            for m in &team.members {
                ...
                lines.push(Line::from(vec![
                    ...
                    Span::raw(format!(" {} (joined {})", m.email, m.joined_at.format("%Y-%m-%d"))),
                ]));
            }
            Paragraph::new(lines)
        } else {
    ...
    frame.render_widget(detail.block(Block::default().borders(Borders::ALL).title(" Team Details ")), chunks[1]);
}
