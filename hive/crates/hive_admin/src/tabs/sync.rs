...
pub fn draw(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    let pct = if stats.storage_available_bytes > 0 {
        (stats.storage_used_bytes as f64 / stats.storage_available_bytes as f64) * 100.0
    } else {
        0.0
    };
    let overview = Paragraph::new(format!(
        ...
        format_bytes(stats.storage_available_bytes),
        pct
    ))
    .block(
        Block::default()
